locals {
  elb_name = format("%s-%s", var.master_prefix, var.elb_name)
  access_logs = length(keys(var.access_logs)) == 0 && var.create_s3 == false ? [] : length(keys(var.access_logs)) == 0 && var.create_s3 ? [{
    bucket  = module.s3.s3_bucket_name
    prefix  = local.elb_name
    enabled = true
  }] : [var.access_logs]
}
...
resource "aws_lb" "this" {
  ...
  security_groups                  = compact(flatten([module.security_group.security_group_id, var.security_groups]))
  subnets                          = length(var.subnet_mapping) > 0 ? null : (var.subnets != null ? var.subnets : tolist(data.aws_subnets.selected.ids))
...
resource "aws_lb_target_group" "main" {
  count = var.create_elb ? length(var.target_groups) : 0
...
resource "aws_lb_target_group_attachment" "this" {
  for_each = var.create_elb && local.target_group_attachments != null ? local.target_group_attachments : {}
...
resource "aws_lb_listener_rule" "https_listener_rule" {
  count = var.create_elb ? length(var.https_listener_rules) : 0

  listener_arn = aws_lb_listener.elb_https[lookup(var.https_listener_rules[count.index], "https_listener_index", count.index)].arn
...
resource "aws_lb_listener_rule" "http_tcp_listener_rule" {
  count = var.create_elb ? length(var.http_tcp_listener_rules) : 0

  listener_arn = aws_lb_listener.elb_http_tcp[lookup(var.http_tcp_listener_rules[count.index], "http_tcp_listener_index", count.index)].arn
...
resource "aws_lb_listener" "elb_http_tcp" {
  count = var.create_elb ? length(var.http_tcp_listeners) : 0
...
resource "aws_lb_listener" "elb_https" {
  count = var.create_elb ? length(var.https_listeners) : 0
...
resource "aws_lb_listener_certificate" "https_listener" {
  count = var.create_elb ? length(var.extra_ssl_certs) : 0

  listener_arn    = aws_lb_listener.elb_https[var.extra_ssl_certs[count.index]["https_listener_index"]].arn
